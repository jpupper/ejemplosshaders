...
vec3 generateBeach(){
	...
	vec3 colarena1 = vec3(r1,g1,b1);
	vec3 colarena2 = vec3(r2,g2,b2);

	vec3 colarenafinal = mix(colarena1,colarena2,noise(uv*10000.,8467.));
	
	float e2 = smoothstep(0.4+e1*0.5,0.1,1.0-uv.y);
	colarenafinal = mix(colarenafinal,vec3(0.),noise(uv*10000.,5284.));
	
	colarenafinal = mix(colarenafinal,vec3(0.),.8);
	vec3 fin = vec3(colarenafinal);
...
void main(void){   
   ...
   vec3 papel = generateBeach();
   
   vec2 uv = vec2(vTexCoord.x,1.-vTexCoord.y);
   vec2 uv2 = vec2(vTexCoord.x,1.-vTexCoord.y);
  
 vec2 uv5 = vec2(vTexCoord.x,1.-vTexCoord.y);
  //uv.x+=sin(time)*.1+.1;
  ...
  float fbm3 = fbm(vec2(uv.x*20.,uv.y*20.),time*.4+524152.);
  uv2 = fract(uv2*2.);
  ...
  vec4 fb = texture2D(feedback,uv2);
  
   vec4 tex = texture2D(tx,uv);
   
   float prom = (tex.r+tex.b+tex.g)/3.;
   vec2 uv3 = vec2(vTexCoord.x,1.-vTexCoord.y); 
   
   
 
   
   vec3 fbm2 = vec3(vec2(uv.x*10000.,uv.y*10000.),time*.2+3921.);
   
     uv3+=prom*0.5;
   
   vec3 fbm1 = vec3(fbm(vec2(uv3.x*20.+fbm2.r,uv3.y*20.+fbm2.y),time*.4+524152.));
		if(rdm(seed*100.0) > .15){
			fbm1 = vec3(fbm(vec2(uv3.x*20.,uv3.y*20.),time*.4+524152.));
		}
   vec3 fin = tex.rgb*1.+fb.rgb*.7;
		fin = mix(tex.rgb,fb.rgb*.7,1.-tex.rgb);
		
		
		fin = mix(fin,fbm1*tex.rgb,fbm1);
		//fin = sin(fin*1.)*.5;
	
	vec2 p = vec2(0.5) -uv5;
	float r = length(p);
	float e = 1.-smoothstep(0.6,0.62,r);
		fin*= vec3(e);
